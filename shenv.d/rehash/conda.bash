...
# Creating shims for those binaries will prevent shenv users to run those
...
conda_exists() {
  shopt -s nullglob
  local condas=($(echo "${SHENV_ROOT}/versions/"*"/bin/conda" "${SHENV_ROOT}/versions/"*"/envs/"*"/bin/conda"))
  shopt -u nullglob
  [ -n "${condas}" ]
}
...
# override `make_shims` to avoid conflict between shenv-virtualenv's `envs.bash`
# https://github.com/shenv/shenv-virtualenv/blob/v20160716/etc/shenv.d/rehash/envs.bash
make_shims() {
  local file shim
  for file do
    shim="${file##*/}"
