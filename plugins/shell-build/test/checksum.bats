...
@test "existing tarball in build location is reused" {
  ...
  export -n SHELL_BUILD_CACHE_PATH
  export SHELL_BUILD_BUILD_PATH="${TMP}/build"

  mkdir -p "$SHELL_BUILD_BUILD_PATH"
  ln -s "${FIXTURE_ROOT}/package-1.0.0.tar.gz" "$SHELL_BUILD_BUILD_PATH"
...
@test "existing tarball in build location is discarded if not matching checksum" {
  ...
  mkdir -p "$SHELL_BUILD_BUILD_PATH"
  touch "${SHELL_BUILD_BUILD_PATH}/package-1.0.0.tar.gz"
